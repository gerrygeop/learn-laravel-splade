<tbody class="divide-y divide-gray-200 bg-white">
   @foreach ($table->resource as $itemKey => $item)
      ...
      <tr @if ($table->rowLinks->has($itemKey)) class="cursor-pointer"
                @click="(event) => table.visit(@js($table->rowLinks->get($itemKey)), @js($table->rowLinkType), event)" @endif
         :class="{
             'bg-gray-50': table.striped && @js($itemKey) % 2,
             'hover:bg-green-200': table.striped,
             'hover:bg-indigo-300': !table.striped
         }">
         ...
         @foreach ($table->columns() as $column)
            <td v-show="table.columnIsVisible(@js($column->key))"
